...
import { getHostingType, isNonCloud } from './analysis/hosting.js';
...
function buildSystemCard(sysList, persona, anchorSystem, allocations, successorName) {
    ...
    sysList.forEach((sys, idx) => {
        ...
        if (persona === 'architect' || persona === 'executive') {
            ...
            const cloudLabel = hostingType === 'cloud' ? 'Cloud' : hostingType === 'partner-hosted' ? 'Partner' : 'On-Prem';
            const cloudTooltip = hostingType === 'cloud' ? 'Cloud-hosted SaaS or platform.' : hostingType === 'partner-hosted' ? `Partner-hosted${sys.hostingPartner ? ' by ' + sys.hostingPartner : ''}.` : 'On-premise council infrastructure.';
            const cloudTagColor = hostingType === 'cloud' ? 'tag-green' : hostingType === 'partner-hosted' ? 'tag-orange' : 'tag-outline';
...
function renderPatternTag(pattern) {
    const isExec = state.activePersona === 'executive' || state.activePersona === 'commercial';
    const patternConfig = {
        'inherit-as-is':                      { colour: 'tag-green',  label: 'Inherit as-is', execLabel: 'No decision needed' },
        'choose-and-consolidate':             { colour: 'tag-blue',   label: 'Choose & consolidate', execLabel: 'Choose one system' },
        'extract-and-partition':               { colour: 'tag-red',    label: 'Extract & partition', execLabel: 'Split across successors' },
        'extract-partition-and-consolidate':   { colour: 'tag-purple', label: 'Extract, partition & consolidate', execLabel: 'Split then choose' }
    };
    const config = patternConfig[pattern] || patternConfig['inherit-as-is'];
    const label = isExec ? config.execLabel : config.label;
    return '<span class="gds-tag ' + config.colour + '">' + label + '</span>';
}
...
function renderPatternTagWithTooltip(pattern) {
    ...
    const patternConfig = {
        'inherit-as-is': {
            ...
            execGuidance: 'Only one system serves this function for this successor. No rationalisation decision is required, though the system may still need review for fitness.'
        },
        'choose-and-consolidate': {
            colour: 'tag-blue',
            label: 'Choose & consolidate',
            execLabel: 'Choose one system',
            ...
            execGuidance: 'Multiple systems from predecessor councils do the same thing. One must be chosen and the others decommissioned — a rationalisation decision is required.'
        },
        'extract-and-partition': {
            colour: 'tag-red',
            label: 'Extract & partition',
            execLabel: 'Split across successors',
            ...
            execGuidance: 'A shared system must be split so each successor authority has its own copy of the data and service. This is complex and time-consuming, especially for large or monolithic systems.'
        },
        'extract-partition-and-consolidate': {
            colour: 'tag-purple',
            label: 'Extract, partition & consolidate',
            execLabel: 'Split then choose',
            ...
            execGuidance: 'A shared system must first be split across successors, then each successor must choose between the resulting systems and alternatives. The most complex pattern — requires sequenced decisions.'
        }
    };
    const config = patternConfig[pattern] || patternConfig['inherit-as-is'];
    const label = isExec ? config.execLabel : config.label;
    const guidance = isExec ? config.execGuidance : config.guidance;
    const tooltip = wrapWithTooltip(label, guidance);
...
document.getElementById('btnLoadDemo').addEventListener('click', () => {
    const demoDataA = {
        councilName: "County Council",
        councilMetadata: { tier: "county", financialDistress: false },
        nodes: [
            { id: "fn-116", label: "Finance", type: "Function", lgaFunctionId: "116" },
            { id: "fn-119", label: "Human Resources", type: "Function", lgaFunctionId: "119" },
            { id: "fn-145", label: "Procurement", type: "Function", lgaFunctionId: "145" },
            { id: "fn-148", label: "Adult Social Care", type: "Function", lgaFunctionId: "148" },
            { id: "fn-109", label: "Highway Maintenance", type: "Function", lgaFunctionId: "109" },
            { id: "fn-65", label: "Public Health", type: "Function", lgaFunctionId: "65" },
            { id: "fn-54", label: "Registration", type: "Function", lgaFunctionId: "54" },
            { id: "fn-152", label: "Children's Social Care", type: "Function", lgaFunctionId: "152" },
            { id: "sys-sap-erp", label: "SAP S/4HANA ERP", type: "ITSystem", vendor: "SAP", users: 5000, cost: "£2,100,000/yr", annualCost: 2100000, endYear: 2030, endMonth: 3, noticePeriod: 18, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: true, supportModel: "vendor-supported", capabilityType: ["payments", "workflow"], version: "S/4HANA 2023", notes: "Monolithic ERP — underpins finance, HR, and procurement. 18-month notice period means decision needed early. On-prem hosting at County Hall DC." },
            { id: "sys-liquidlogic", label: "Liquidlogic LAS", type: "ITSystem", vendor: "System C", users: 1200, cost: "£480,000/yr", annualCost: 480000, endYear: 2028, endMonth: 9, noticePeriod: 6, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported", version: "v14.2", notes: "Good data portability via SSDA903 exports. Consider early engagement with System C about multi-authority licensing." },
            { id: "sys-confirm-highways", label: "Confirm Highways", type: "ITSystem", vendor: "Confirm", users: 400, cost: "£320,000/yr", annualCost: 320000, endYear: 2029, endMonth: 6, noticePeriod: 6, portability: "Medium", dataPartitioning: "Segmented", hosting: "on-premise", isERP: false, supportModel: "vendor-supported", sharedWith: ["District Council 1"], version: "v22.10", notes: "Shared instance with District 1 — both councils contribute asset data. GIS integration with ESRI." },
            { id: "sys-public-health-intel", label: "Public Health Intelligence", type: "ITSystem", vendor: "In-House", users: 80, cost: "£35,000/yr", annualCost: 35000, endYear: 2030, endMonth: 12, noticePeriod: 3, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "community-supported", sharedWith: ["Other County Programme Council A", "Other County Programme Council B"], version: "v2.1", notes: "Developed in-house by county digital team. Python/Django stack. Two other programme councils contribute data and co-fund development." },
            { id: "sys-nec-revenues", label: "NEC Revenues", type: "ITSystem", vendor: "NEC", users: 300, cost: "£190,000/yr", annualCost: 190000, endYear: 2026, endMonth: 6, noticePeriod: 12, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "vendor-supported", version: "NEC iWorld 2019", notes: "Contract expires before vesting date — must either extend or migrate. Notice period means decision needed imminently." },
            { id: "sys-capita-one", label: "Capita ONE", type: "ITSystem", vendor: "Capita", users: 800, cost: "£350,000/yr", annualCost: 350000, endYear: 2028, endMonth: 12, noticePeriod: 6, portability: "Medium", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported", version: "v22.3", notes: "Critical for children's services statutory returns. ADCS data standards compliance." }
        ],
        edges: [
            { source: "sys-sap-erp", target: "fn-116", relationship: "REALIZES" },
            { source: "sys-sap-erp", target: "fn-119", relationship: "REALIZES" },
            { source: "sys-sap-erp", target: "fn-145", relationship: "REALIZES" },
            { source: "sys-liquidlogic", target: "fn-148", relationship: "REALIZES" },
            { source: "sys-liquidlogic", target: "sys-sap-erp", relationship: "CONSUMES_CAPABILITY", capabilities: ["payments"] },
            { source: "sys-confirm-highways", target: "fn-109", relationship: "REALIZES" },
            { source: "sys-public-health-intel", target: "fn-65", relationship: "REALIZES" },
            { source: "sys-nec-revenues", target: "fn-54", relationship: "REALIZES" },
            { source: "sys-capita-one", target: "fn-152", relationship: "REALIZES" }
        ]
    };
    const demoDataB = {
        councilName: "District Council 1",
        councilMetadata: { tier: "district", financialDistress: false },
        nodes: [
            { id: "fn-116", label: "Finance", type: "Function", lgaFunctionId: "116" },
            { id: "fn-3", label: "Benefits", type: "Function", lgaFunctionId: "3" },
            { id: "fn-101", label: "Development Control", type: "Function", lgaFunctionId: "101" },
            { id: "fn-38", label: "Waste Management", type: "Function", lgaFunctionId: "38" },
            { id: "fn-43", label: "Accessing Information", type: "Function", lgaFunctionId: "43" },
            { id: "fn-109", label: "Highway Maintenance", type: "Function", lgaFunctionId: "109" },
            { id: "sys-civica-fin", label: "Civica Financials", type: "ITSystem", vendor: "Civica", users: 120, cost: "£95,000/yr", annualCost: 95000, endYear: 2028, endMonth: 6, noticePeriod: 6, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported", version: "Civica Cx 2024", notes: "Recently upgraded from legacy Authority system. Clean data model." },
            { id: "sys-nec-revben", label: "NEC Revenues & Benefits", type: "ITSystem", vendor: "NEC", users: 200, cost: "£180,000/yr", annualCost: 180000, endYear: 2027, endMonth: 3, noticePeriod: 9, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "vendor-supported", sharedWith: ["District Council 2"], version: "NEC iWorld 2019", notes: "Shared instance with District 2. Contract held by District 1. Monolithic data — disaggregation would require NEC professional services." },
            { id: "sys-idox-uniform", label: "Idox Uniform", type: "ITSystem", vendor: "Idox", users: 35, cost: "£55,000/yr", annualCost: 55000, endYear: 2029, endMonth: 3, noticePeriod: 3, portability: "Medium", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-bartec-waste", label: "Bartec Waste Management", type: "ITSystem", vendor: "Bartec", users: 90, cost: "£130,000/yr", annualCost: 130000, endYear: 2029, endMonth: 6, noticePeriod: 6, portability: "Medium", dataPartitioning: "Segmented", hosting: "on-premise", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-dynamics-crm", label: "Microsoft Dynamics 365", type: "ITSystem", vendor: "Microsoft", users: 350, cost: "£110,000/yr", annualCost: 110000, endYear: 2030, endMonth: 9, noticePeriod: 3, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported", capabilityType: ["forms"] },
            { id: "sys-confirm-highways-d1", label: "Confirm Highways", type: "ITSystem", vendor: "Confirm", users: 45, cost: "£95,000/yr", annualCost: 95000, endYear: 2029, endMonth: 6, noticePeriod: 6, portability: "Medium", dataPartitioning: "Segmented", hosting: "partner-hosted", hostingPartner: "County Council", isERP: false, supportModel: "vendor-supported", sharedWith: ["County Council"] }
        ],
        edges: [
            { source: "sys-civica-fin", target: "fn-116", relationship: "REALIZES" },
            { source: "sys-nec-revben", target: "fn-3", relationship: "REALIZES" },
            { source: "sys-idox-uniform", target: "fn-101", relationship: "REALIZES" },
            { source: "sys-bartec-waste", target: "fn-38", relationship: "REALIZES" },
            { source: "sys-dynamics-crm", target: "fn-43", relationship: "REALIZES" },
            { source: "sys-confirm-highways-d1", target: "fn-109", relationship: "REALIZES" }
        ]
    };
    const demoDataC = {
        councilName: "District Council 2",
        councilMetadata: { tier: "district", financialDistress: false },
        nodes: [
            { id: "fn-3", label: "Benefits", type: "Function", lgaFunctionId: "3" },
            { id: "fn-66", label: "Housing", type: "Function", lgaFunctionId: "66" },
            { id: "fn-101", label: "Development Control", type: "Function", lgaFunctionId: "101" },
            { id: "fn-38", label: "Waste Management", type: "Function", lgaFunctionId: "38" },
            { id: "fn-43", label: "Accessing Information", type: "Function", lgaFunctionId: "43" },
            { id: "sys-nec-revben-d2", label: "NEC Revenues & Benefits", type: "ITSystem", vendor: "NEC", users: 150, cost: "£160,000/yr", annualCost: 160000, endYear: 2027, endMonth: 3, noticePeriod: 9, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "vendor-supported", sharedWith: ["District Council 1"] },
            { id: "sys-nec-housing", label: "NEC Housing", type: "ITSystem", vendor: "NEC", users: 100, cost: "£140,000/yr", annualCost: 140000, endYear: 2028, endMonth: 12, noticePeriod: 6, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-nec-planning", label: "NEC Planning", type: "ITSystem", vendor: "NEC", users: 28, cost: "£60,000/yr", annualCost: 60000, endYear: 2029, endMonth: 9, noticePeriod: 6, portability: "Medium", dataPartitioning: "Segmented", hosting: "on-premise", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-whitespace-waste", label: "Whitespace Waste", type: "ITSystem", vendor: "Whitespace", users: 60, cost: "£85,000/yr", annualCost: 85000, endYear: 2028, endMonth: 3, noticePeriod: 3, portability: "Medium", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-firmstep-crm", label: "Firmstep Digital", type: "ITSystem", vendor: "Firmstep", users: 200, cost: "£75,000/yr", annualCost: 75000, endYear: 2030, endMonth: 6, noticePeriod: 3, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported" }
        ],
        edges: [
            { source: "sys-nec-revben-d2", target: "fn-3", relationship: "REALIZES" },
            { source: "sys-nec-housing", target: "fn-66", relationship: "REALIZES" },
            { source: "sys-nec-planning", target: "fn-101", relationship: "REALIZES" },
            { source: "sys-whitespace-waste", target: "fn-38", relationship: "REALIZES" },
            { source: "sys-firmstep-crm", target: "fn-43", relationship: "REALIZES" }
        ]
    };
    const demoDataD = {
        councilName: "District Council 3",
        ...
        nodes: [
            ...
            { id: "fn-66", label: "Housing", type: "Function", lgaFunctionId: "66" },
            { id: "fn-43", label: "Accessing Information", type: "Function", lgaFunctionId: "43" },
            { id: "fn-76", label: "Libraries", type: "Function", lgaFunctionId: "76" },
            { id: "sys-sage-finance", label: "Sage 50 Finance", type: "ITSystem", vendor: "Sage", users: 25, cost: "£15,000/yr", annualCost: 15000, endYear: 2026, endMonth: 3, noticePeriod: 6, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "unsupported" },
            { id: "sys-inhouse-planning", label: "In-House Planning Register", type: "ITSystem", vendor: "In-House", users: 15, cost: "£12,000/yr", annualCost: 12000, endYear: 2027, endMonth: 6, noticePeriod: 3, portability: "Medium", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "unsupported" },
            { id: "sys-inhouse-waste", label: "In-House Waste Scheduling", type: "ITSystem", vendor: "In-House", users: 20, cost: "£8,000/yr", annualCost: 8000, endYear: 2028, endMonth: 9, noticePeriod: 3, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "unsupported" },
            { id: "sys-nec-housing-d3", label: "NEC Housing", type: "ITSystem", vendor: "NEC", users: 60, cost: "£90,000/yr", annualCost: 90000, endYear: 2029, endMonth: 3, noticePeriod: 6, portability: "Low", dataPartitioning: "Monolithic", hosting: "on-premise", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-goss-icm", label: "GOSS iCM", type: "ITSystem", vendor: "GOSS", users: 100, cost: "£40,000/yr", annualCost: 40000, endYear: 2030, endMonth: 12, noticePeriod: 3, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "vendor-supported" },
            { id: "sys-koha-library", label: "Koha Library System", type: "ITSystem", vendor: "In-House", users: 30, cost: "£18,000/yr", annualCost: 18000, endYear: 2030, endMonth: 9, noticePeriod: 3, portability: "High", dataPartitioning: "Segmented", hosting: "cloud", isERP: false, supportModel: "community-supported", sharedWith: ["Neighbouring Council A", "Neighbouring Council B"] }
        ],
        edges: [
            { source: "sys-sage-finance", target: "fn-116", relationship: "REALIZES" },
            { source: "sys-inhouse-planning", target: "fn-101", relationship: "REALIZES" },
            { source: "sys-inhouse-waste", target: "fn-38", relationship: "REALIZES" },
            { source: "sys-nec-housing-d3", target: "fn-66", relationship: "REALIZES" },
            { source: "sys-goss-icm", target: "fn-43", relationship: "REALIZES" },
            { source: "sys-koha-library", target: "fn-76", relationship: "REALIZES" }
        ]
    };

    state.rawUploads = [
        { filename: 'county-council.json', data: demoDataA },
        { filename: 'district-council-1.json', data: demoDataB },
        { filename: 'district-council-2.json', data: demoDataC },
        { filename: 'district-council-3.json', data: demoDataD }
    ];

    // Also set pending transition config
    state.pendingTransitionConfig = {
        vestingDate: "2027-04-01",
        successors: [
            {
                name: "New Unitary Authority",
                fullPredecessors: ["District Council 1", "District Council 2", "District Council 3"],
                partialPredecessors: ["County Council"]
            }
        ]
    };
    ...
    listUl.innerHTML = '';
    [
        { name: 'County Council', idx: 0, count: 14 },
        { name: 'District Council 1', idx: 1, count: 12 },
        { name: 'District Council 2', idx: 2, count: 10 },
        { name: 'District Council 3', idx: 3, count: 12 }
    ].forEach(c => {
        const li = document.createElement('li');
        li.className = 'flex items-center gap-3';
        const span = document.createElement('span');
        span.textContent = `${c.name} (${c.count} nodes)`;
        const editBtn = document.createElement('button');
        editBtn.className = 'btn-edit-arch gds-btn-secondary px-2 py-1 text-xs font-bold hover:bg-gray-100';
