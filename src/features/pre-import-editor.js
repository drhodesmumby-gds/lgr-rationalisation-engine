...
function buildSystemsTabHtml(nodes, edges) {
    ...
    <td class="px-2 py-1.5"><select class="editor-sys-field border border-gray-300 p-1 text-xs" data-node-idx="${globalIdx}" data-field="hosting"><option value="">--</option><option value="cloud"${(sys.hosting === 'cloud') ? ' selected' : ''}>Cloud</option><option value="on-premise"${(sys.hosting === 'on-premise') ? ' selected' : ''}>On-premise</option><option value="partner-hosted"${sys.hosting === 'partner-hosted' ? ' selected' : ''}>Partner-hosted</option></select></td>
